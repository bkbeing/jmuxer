...
var OpusParser = function () {
    createClass(OpusParser, null, [{
        key: 'extractOpus',
        value: function extractOpus(buffer) {
            /*let i = 0,
                length = buffer.byteLength,
                result = [],
                headerLength,
    ...
    function OpusParser(remuxer) {
        classCallCheck(this, OpusParser);

        this.remuxer = remuxer;
        this.track = remuxer.mp4track;
    }

    createClass(OpusParser, [{
        key: 'setOpusConfig',
        value: function setOpusConfig() {
            /*let objectType,
                sampleIndex,
                channelCount,
                config = new Uint8Array(2),
...
var MP4 = function () {
    ...
    createClass(MP4, null, [{
        ...
        value: function opus(track) {
            var audiosamplerate = track.audiosamplerate;
            return MP4.box(MP4.types.Opus, new Uint8Array([0x00, 0x00, 0x00, // reserved
            0x00, 0x00, 0x00, // reserved
            0x00, 0x01, // data_reference_index
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // reserved
            0x00, track.channelCount, // channelcount
        ...
        value: function dops(track) {
            ...
            return MP4.box(MP4.types.dOps, new Uint8Array([0x00, // version
        ...
        value: function stsd(track) {
            if (track.type === 'audio') {
                if (track.codec.startsWith('mp4a')) {
                    ...
                    error('Unknown track codec ' + track.codec);
                }
            } else {
        ...
        key: 'initSegmentMin',
        value: function initSegmentMin(tracks, duration, timescale) {
            if (!MP4.types) {
                MP4.init();
            }
            var result = new Uint8Array(MP4.FTYP.byteLength);
            result.set(MP4.FTYP);
            return result;
        }
    }]);
...
var OpusRemuxer = function (_BaseRemuxer) {
    inherits(OpusRemuxer, _BaseRemuxer);

    function OpusRemuxer() {
        classCallCheck(this, OpusRemuxer);

        var _this = possibleConstructorReturn(this, (OpusRemuxer.__proto__ || Object.getPrototypeOf(OpusRemuxer)).call(this));
        ...
        _this.samples = [];
        _this.opus = new OpusParser(_this);
        return _this;
    }

    createClass(OpusRemuxer, [{
        key: 'resetTrack',
        value: function resetTrack() {
            this.readyToDecode = false;
            this.mp4track.codec = '';
...
function appendByteArray(buffer1, buffer2, buffer3) {
    var length = (buffer1.byteLength | 0) + (buffer2.byteLength | 0);
    if (buffer3) {
...
var RemuxController = function (_Event) {
    ...
    function RemuxController(streaming, mp4Type) {
        ...
        _this.mp4Type = mp4Type;
        return _this;
    }

    createClass(RemuxController, [{
        key: 'addTrack',
        value: function addTrack(type, audioCodec) {
            ...
            if (type === 'audio' || type === 'both') {
                if (audioCodec === AACParser.codec) {
                    ...
                    error('Unknown audio codev audioCodec');
                }
            }
        }
    }, {
            ...
            if (this.mp4Type === 'fragmented' || this.mp4Type === 'combined') {
                if (!this.initialized) {
                        ...
                        if (this.mp4Type === 'fragmented') {
                            ...
                            try {
                                for (var _iterator3 = this.trackTypes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                                    ...
                                    allTracks.push(_track.mp4track);
                                }
                            } catch (err) {
                                _didIteratorError3 = true;
                                _iteratorError3 = err;
                    ...
                    try {
                        for (var _iterator4 = this.trackTypes[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                            ...
                            var _track2 = this.tracks[_type2];
                            var pay = _track2.getPayload();
                            if (pay && pay.byteLength) {
                                var moof = MP4.moof(_track2.seq, _track2.dts, _track2.mp4track);
                                var mdat = MP4.mdat(pay);
                                var _payload = appendByteArray(moof, mdat);
                                var _data2 = {
                                    type: _type2,
                                    payload: _payload,
                                    dts: _track2.dts
                                };
                                this.dispatch('buffer', _data2);
                                var duration = secToTime(_track2.dts / 1000);
                                log('put segment (' + _type2 + '): ' + _track2.seq + ' dts: ' + _track2.dts + ' samples: ' + _track2.mp4track.samples.length + ' second: ' + duration);
                                _track2.flush();
                            }
                        }
                    } catch (err) {
                ...
                try {
                    for (var _iterator5 = this.trackTypes[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                        var _type3 = _step5.value;

                        _allTracks.push(this.tracks[_type3].mp4track);
                    }
                } catch (err) {
                ...
                var initSegment = MP4.initSegmentMin(_allTracks, this.mediaDuration, _allTracks[0].timescale);
                ...
                try {
                    for (var _iterator6 = this.trackTypes[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
                        var _type4 = _step6.value;

                        var _track3 = this.tracks[_type4];

                        trackPayload = MP4.mdat(_track3.getPayload());
                        if (!payload) {
                ...
                var moov = MP4.moov(_allTracks, this.mediaDuration, _allTracks[0].timescale);
                var _data3 = {
                    payload: appendByteArray(initSegment, payload, moov)
                };

                this.dispatch('buffer', _data3);
            } else {
                error('Unknown mp4 type ' + this.mp4Type);
            }
        }
    }, {
        key: 'isReady',
        value: function isReady() {
            ...
            try {
                for (var _iterator7 = this.trackTypes[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
        ...
        value: function remux(data) {
            ...
            try {
                for (var _iterator8 = this.trackTypes[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
...
var JMuxmer = function (_Event) {
    ...
    function JMuxmer(options) {
        ...
        _this.setupMSE();
        _this.remuxController = new RemuxController(_this.options.clearBuffer, _this.options.mp4Type);
        _this.remuxController.addTrack(_this.options.mode, _this.options.audioCodec);
